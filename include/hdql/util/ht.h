...
HDQL_API void hdql_ht_destroy(struct hdql_ht *ht);
 ...
 * Returns 0 when done.
 * */
HDQL_API int hdql_ht_iter( const struct hdql_ht * ht
...
HDQL_API int hdql_ht_rebuild(struct hdql_ht *ht);

/**\brief Returns number of entries in hash table */
HDQL_API size_t hdql_ht_size(const struct hdql_ht *ht);
