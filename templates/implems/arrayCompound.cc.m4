...
_`'TYPE`'_`'ATTR_NAME`'_dereference( hdql_It_t it_
    ...
    return reinterpret_cast<hdql_Datum_t>(it.it->get());
}
    ...
    do {
        ++it.it;
         ...
         && ( it.it != it.ownerPtr->ATTR_NAME.end()
          && ( it.it->first < static_cast<size_t>(selection->nMin)
            || it.it->first > static_cast<size_t>(selection->nMax)
             )));
...
_`'TYPE`'_`'ATTR_NAME`'_reset( hdql_It_t it_
    ...
    if(selection) {
        while( it.it != it.ownerPtr->ATTR_NAME.end()
            && (static_cast<ssize_t>(selection->nMin) > it.it->first
             || static_cast<ssize_t>(selection->nMax) < it.it->first)) {
            ++it.it;
        }
    }
    return reinterpret_cast<hdql_It_t>(&it);
