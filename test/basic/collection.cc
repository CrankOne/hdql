...
hdql_Datum_t
create_iterator( hdql_Datum_t newOwner
    ...
    if(sel) throw std::runtime_error("unexpected selection");
    if(!context) throw std::runtime_error("no context");
    int **it = new int*;
    return reinterpret_cast<hdql_Datum *>(it);
}

hdql_Datum_t
dererefence_iterator(hdql_Datum_t it_) {
    if(!it_) throw std::runtime_error("NULL iterator");
    int ** it = reinterpret_cast<int**>(it_);
    return reinterpret_cast<hdql_Datum_t>(*it);
}

hdql_It_t
advance_iterator(hdql_It_t, struct hdql_Key *k) {
    // ... cna we have defData here?
}
...
hdql_ScalarAttrInterface statefulScalarAttrIFace {
    ...
    .dereference = dererefence_a_stateful,  // XXX, to be removed
