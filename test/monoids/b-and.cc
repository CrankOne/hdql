...
TEST_F(TestMonoidal, bANDRefusesFloatingPointType) {
    ...
    _query = hdql_compile_query("bAND(.a.ff)", _rootCompound, _compoundsContext
...
TEST_F(TestMonoidal, bANDRefusesDoubleType) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("bAND(.a.df)", _rootCompound, _compoundsContext
            , errBuf, sizeof(errBuf), errDetails );
    EXPECT_FALSE(_query);
...
TEST_F(TestMonoidal, bANDRefusesBooleanType) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("bAND(.a.bf)", _rootCompound, _compoundsContext
            , errBuf, sizeof(errBuf), errDetails );
    EXPECT_FALSE(_query);
    EXPECT_EQ( errDetails[0]
             , HDQL_ERR_TRANSLATION_FAILURE
...
TEST_F(TestMonoidal, bANDRefusesCompoundType) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("bAND(.a)", _rootCompound, _compoundsContext
            , errBuf, sizeof(errBuf), errDetails );
    EXPECT_FALSE(_query);
    ASSERT_EQ( errDetails[0]
             , HDQL_ERR_TRANSLATION_FAILURE
