...
TEST_F(TestMonoidal, bXORRefusesFloatingPointType) {
    ...
    _query = hdql_compile_query("bXOR(.a.ff)", _rootCompound, _compoundsContext
...
TEST_F(TestMonoidal, bXORRefusesDoubleType) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("bXOR(.a.df)", _rootCompound, _compoundsContext
            , errBuf, sizeof(errBuf), errDetails );
    EXPECT_FALSE(_query);
...
TEST_F(TestMonoidal, bXORRefusesBooleanType) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("bXOR(.a.bf)", _rootCompound, _compoundsContext
            , errBuf, sizeof(errBuf), errDetails );
    EXPECT_FALSE(_query);
    EXPECT_EQ( errDetails[0]
             , HDQL_ERR_TRANSLATION_FAILURE
...
TEST_F(TestMonoidal, bXORRefusesCompoundType) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("bXOR(.a)", _rootCompound, _compoundsContext
            , errBuf, sizeof(errBuf), errDetails );
    EXPECT_FALSE(_query);
    ASSERT_EQ( errDetails[0]
             , HDQL_ERR_TRANSLATION_FAILURE
