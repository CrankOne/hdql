...
TEST_F(TestAggFuncs, countTypeInQueryResultsInAKeylessBooleanScalar) {
    ...
    ASSERT_TRUE(types);
    hdql_ValueTypeCode_t booltc = hdql_types_get_type_code(types, "uint64_t");
...
TEST_F(TestAggFuncs, countRefusesCompoundType) {
    using namespace hdql::test;
    RootItem root;
...
TEST_F(TestAggFuncs, countOfAnEmptyCollectionIsZero) {
    using namespace hdql::test;
    RootItem root;
...
TEST_F(TestAggFuncs, countOfASingleTrueElement) {
    using namespace hdql::test;
    RootItem root;
...
TEST_F(TestAggFuncs, countOfASingleFalseElement) {
    using namespace hdql::test;
    RootItem root;
    std::shared_ptr<Item> item1 = std::make_shared<Item>();
    item1->u16f = 0x0;
...
TEST_F(TestAggFuncs, countAccountsNAN) {
    using namespace hdql::test;
    RootItem root;
    std::shared_ptr<Item> item1 = std::make_shared<Item>();
...
TEST_F(TestAggFuncs, countOfASequence) {
    using namespace hdql::test;
    RootItem root;
...
TEST_F(TestAggFuncs, countOfTwoSequences) {
    using namespace hdql::test;
    RootItem root;
