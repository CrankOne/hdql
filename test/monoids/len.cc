...
//    _query = hdql_compile_query(".a{l:=len(.bf)}.l", _rootCompound, _compoundsContext
...
TEST_F(TestMonoidal, lenApplicableToScalarAttrOfACollection) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("len(.a.bf)", _rootCompound, _compoundsContext
            , errBuf, sizeof(errBuf), errDetails );
    ASSERT_TRUE(_query);
...
TEST_F(TestMonoidal, lenRefusesMultipleArguments) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("len(.a, .b)", _rootCompound, _compoundsContext
            , errBuf, sizeof(errBuf), errDetails );
    EXPECT_FALSE(_query);
