...
TEST_F(TestAggFuncs, minRefusesBooleanType) {
    ...
    _query = hdql_compile_query("min(.a.bf)", _rootCompound, _compounds.context_ptr()
...
TEST_F(TestAggFuncs, minRefusesCompoundType) {
    ...
    RootItem root;
    char errBuf[128]; int errDetails[5];
    _query = hdql_compile_query("min(.a)", _rootCompound, _compounds.context_ptr()
            , errBuf, sizeof(errBuf), errDetails );
    EXPECT_FALSE(_query);
